...
@NSApplicationMain
class AppDelegate: NSObject, NSApplicationDelegate, NSCollectionViewDataSource,
  NSCollectionViewDelegate, NSDraggingDestination
{
  ...
  func applicationDidFinishLaunching(_ aNotification: Notification) {
    ...
    // Setup preferences defaults FIRST (before anything else)
  ...
  private func setupStatusItem() {
    ...
    print("DEBUG: ShowStatusIcon preference = \(showStatusIcon)")

    if showStatusIcon {
      // Create status item
      statusItem = NSStatusBar.system.statusItem(withLength: NSStatusItem.variableLength)
      print("DEBUG: Created status item: \(statusItem != nil)")
      ...
      if let button = statusItem.button {
        ...
        print("DEBUG: Status item button configured with title: \(button.title ?? "nil")")
      } else {
        print("DEBUG: ERROR - Status item button is nil!")
      }
    } else {
      print("DEBUG: Status icon is disabled in preferences")
    }
  }
  ...
  private func setupFloatingPanel() {
    ...
    appListView.dataSource = self
    appListView.delegate = self

    // Register collection view item class
    appListView.register(
      AppCollectionViewItem.self,
      forItemWithIdentifier: NSUserInterfaceItemIdentifier("AppCollectionViewItem"))
  ...
  func collectionView(
    ...
    let item =
      collectionView.makeItem(
        withIdentifier: NSUserInterfaceItemIdentifier("AppCollectionViewItem"), for: indexPath)
      as! AppCollectionViewItem
  ...
  func collectionView(_ collectionView: NSCollectionView, pasteboardWriterForItemAt index: Int)
    -> NSPasteboardWriting?
  {
    let pasteboardItem = NSPasteboardItem()
    pasteboardItem.setString(String(index), forType: .string)
    if let bundleID = managedApps[index].bundleIdentifier {
      pasteboardItem.setString(bundleID, forType: .string)
    }
  ...
  func collectionView(
    ...
    willBeginAt screenPoint: NSPoint, forItemsAt indexes: IndexSet
  ) {
  ...
  func performDragOperation(_ sender: NSDraggingInfo) -> Bool {
    ...
    if let bundleIDs = pasteboard.readObjects(forClasses: [NSString.self], options: nil)
      as? [String]
    {
